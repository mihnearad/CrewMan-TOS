...
const statuses = [
  ...
  { value: 'planned', label: 'Planned', bgColor: 'bg-yellow-100', textColor: 'text-yellow-800' },
  { value: 'completed', label: 'Completed', bgColor: 'bg-gray-100', textColor: 'text-gray-800' },
  { value: 'cancelled', label: 'Cancelled', bgColor: 'bg-red-100', textColor: 'text-red-800' },
]

export default function QuickStatusBadge({ projectId, currentStatus }: QuickStatusBadgeProps) {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {isOpen && (
        <div className="absolute z-10 mt-1 w-36 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5">
          <div className="py-1">
            {statuses.map((status) => (
              <button
                key={status.value}
